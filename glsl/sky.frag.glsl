...
//    return;
...
#ifdef RAY_AS_COLOR
    outColor = 0.5 * vec4(rayDir + vec3(1,1,1), 1);
    return;
#endif
    ...
    vec4 sunsetColor = vec4(uvToSunset(uv + offset * 0.1), 1);
    vec4 duskColor = vec4(uvToDusk(uv + offset * 0.1), 1);
    ...
    if(angle < sunSize) {
        ...
        // Corona of sun, mix with sky color
            ...
            outColor = mix(vec4(sunColor, 1), sunsetColor, (angle - 7.5) / 22.5);
        }
    }
